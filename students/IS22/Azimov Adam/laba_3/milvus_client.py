...
class MilvusClient:
    ...
    def list_collections(self, with_info: bool = False) -> List[Any]:
        """
        Получение списка коллекций в Milvus.

        Args:
            with_info: Если True, возвращает подробную информацию о коллекциях

        Returns:
            Если with_info=False: список имен коллекций
            Если with_info=True: список словарей с информацией о коллекциях
        """
        try:
            ...
            # Получаем подробную информацию о каждой коллекции
            collections_info = []
            for collection_name in collections:
                try:
                    ...
                    # Получаем количество записей (нужно загрузить коллекцию)
                    try:
                        collection.load()
                    ...
                    # Получаем информацию о полях схемы
                    fields_info = []
                    for field in schema.fields:
                        ...
                        if hasattr(field, "params") and field.params:
                            field_info.update(field.params)
                    ...
                    # Если не удалось получить информацию о коллекции,
    ...
    def create_collection(self, collection_name: str, dimension: int, description: str = "", metric_type: str = "COSINE") -> Collection:
        ...
        fields = [
            ...
            FieldSchema(name="text", dtype=DataType.VARCHAR, max_length=65535, description="Исходный текст"),
            FieldSchema(name="embedding", dtype=DataType.FLOAT_VECTOR, dim=dimension, description="Векторное представление текста"),
            FieldSchema(name="file_name", dtype=DataType.VARCHAR, max_length=512, description="Название файла-источника"),
            FieldSchema(name="file_path", dtype=DataType.VARCHAR, max_length=1024, description="Полный путь к файлу-источнику"),
            FieldSchema(name="chunk_index", dtype=DataType.INT64, description="Индекс чанка в документе (начинается с 0)"),
        ]
        ...
        index_params = {"metric_type": metric_type, "index_type": "IVF_FLAT", "params": {"nlist": 128}}
    ...
    def insert_data(
            ...
            try:
                ...
                if embedding_field:
                    ...
                    if actual_dim != expected_dim:
                        raise ValueError(f"Неверная размерность embeddings: ожидается {expected_dim}, получено {actual_dim}")
        ...
        entities = [texts, embeddings, file_names, file_paths, chunk_indices]
    ...
    def search(self, collection_name: str, query_vectors: List[List[float]], top_k: int = 5, expr: Optional[str] = None) -> List[List[dict]]:
        """
        ...
        search_params = {"metric_type": metric_type, "params": {"nprobe": 10}}  # Количество кластеров для поиска
                ...
                hits.append(
                    {
                        ...
                        "file_name": entity.file_name if hasattr(entity, "file_name") else "",
                        "file_path": entity.file_path if hasattr(entity, "file_path") else "",
                        "chunk_index": entity.chunk_index if hasattr(entity, "chunk_index") else -1,
                    }
                )
    ...
    def get_collection_info(self, collection_name: str) -> dict:
        ...
        try:
            ...
            return {"exists": True, "num_entities": collection.num_entities, "schema": collection.schema, "indexes": collection.indexes}
        except Exception as e:
            return {"exists": True, "error": f"Ошибка при получении информации: {e}"}
    ...
    def get_document_chunks(self, collection_name: str, file_path: str, order_by_index: bool = True) -> List[dict]:
        """
        ...
        try:
            results = collection.query(expr=expr, output_fields=["id", "text", "file_name", "file_path", "chunk_index"])
        except Exception as e:
    ...
    def reconstruct_document(self, collection_name: str, file_path: str) -> str:
        """
