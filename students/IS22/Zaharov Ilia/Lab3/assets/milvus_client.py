...
class MilvusClient:
    ...
    def _utility_call(self, fn, *args, **kwargs):
        """
        В разных версиях pymilvus у utility.* разные сигнатуры.
        Пытаемся прокинуть using=self.alias, а если параметр не поддерживается — вызываем без него.
        """
        try:
            return fn(*args, using=self.alias, **kwargs)
        except TypeError:
    ...
    def create_collection(
        ...
        if self._utility_call(utility.has_collection, collection_name):
    ...
    def insert_data(
        ...
        """
        if not self._utility_call(utility.has_collection, collection_name):
    ...
    def get_collection_info(self, collection_name: str) -> dict:
        ...
        try:
            ...
            info = {
                ...
                # DRF/JSON не умеет сериализовывать объекты pymilvus (CollectionSchema/Index)
                "schema": self._collection_schema_to_dict(collection.schema),
                "indexes": [self._index_to_dict(idx) for idx in (collection.indexes or [])],
            }
            # На случай, если в будущих версиях pymilvus появятся не-JSON типы глубже,
            # приводим весь ответ к JSON-совместимому виду.
    ...
    def _jsonable(value):
        ...
        Нужен для параметров схемы/индексов, где встречаются enum'ы и вложенные структуры.
        """
        if value is None or isinstance(value, (str, int, float, bool)):
            return value

        # Enum / IntEnum (например DataType)
        if isinstance(value, Enum):
            return value.name

        if isinstance(value, (list, tuple, set)):
            return [MilvusClient._jsonable(v) for v in value]

        if isinstance(value, dict):
            return {str(k): MilvusClient._jsonable(v) for k, v in value.items()}

        # Некоторые объекты pymilvus могут иметь to_dict()
        to_dict = getattr(value, "to_dict", None)
        if callable(to_dict):
            try:
                return MilvusClient._jsonable(to_dict())
            except Exception:
                pass

        # Фолбэк — строковое представление (лучше, чем падать на JSON сериализации)
        return str(value)
    ...
    def _field_schema_to_dict(field: FieldSchema) -> dict:
        ...
        return {
            ...
            "params": MilvusClient._jsonable(getattr(field, "params", {}) or {}),
        }

    @staticmethod
    def _collection_schema_to_dict(schema: CollectionSchema) -> dict:
        fields = getattr(schema, "fields", None) or []
        return {
            "description": getattr(schema, "description", "") or "",
            "fields": [MilvusClient._field_schema_to_dict(f) for f in fields],
            # Опциональные поля (в зависимости от версии pymilvus)
            "enable_dynamic_field": bool(getattr(schema, "enable_dynamic_field", False)),
        }

    @staticmethod
    def _index_to_dict(index) -> dict:
        # В разных версиях pymilvus это может быть разный класс,
        # поэтому используем getattr и приводим параметры к JSON.
        return {
            "field_name": getattr(index, "field_name", None),
            "index_name": getattr(index, "index_name", None),
            "params": MilvusClient._jsonable(getattr(index, "params", {}) or {}),
        }
    
    def delete_collection(self, collection_name: str):
        """Удаление коллекции."""
        if self._utility_call(utility.has_collection, collection_name):
            self._utility_call(utility.drop_collection, collection_name)
            print(f"Коллекция '{collection_name}' удалена")
        else:
            print(f"Коллекция '{collection_name}' не существует")

    def list_collections(self) -> List[str]:
        """Список коллекций в Milvus."""
        return list(self._utility_call(utility.list_collections))

    def has_collection(self, collection_name: str) -> bool:
        """Проверка существования коллекции."""
        return bool(self._utility_call(utility.has_collection, collection_name))
