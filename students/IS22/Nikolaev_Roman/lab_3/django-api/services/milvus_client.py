...
class MilvusClient:
    ...
    def create_collection(
        ...
        print(
            f"Коллекция '{collection_name}' создана (dim={dimension}, metric={metric_type})"
        )
        return collection
    ...
    def insert_data(
        ...
        if len(texts) != len(embeddings):
            raise ValueError(
                "Количество текстов должно совпадать с количеством векторов"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"Не удалось загрузить коллекцию '{collection_name}': {e}"
            )
        ...
        if (
            len(file_names) != num_items
            or len(file_paths) != num_items
            or len(chunk_indices) != num_items
        ):
            raise ValueError(
                "Длины всех списков метаданных должны совпадать с количеством текстов"
            )
    ...
    def get_document_chunks(
        self, collection_name: str, file_path: str, order_by_index: bool = True
    ) -> List[dict]:
        """
    ...
    def reconstruct_document(self, collection_name: str, file_path: str) -> str:
        ...
        chunks = self.get_document_chunks(
            collection_name, file_path, order_by_index=True
        )
