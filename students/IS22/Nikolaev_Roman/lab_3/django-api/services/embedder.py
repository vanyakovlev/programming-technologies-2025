...
class Embedder:
    ...
    def encode_query(self, query: str, normalize: bool = True) -> List[float]:
        ...
        embedding = self.model.encode(
            prefixed_query, normalize_embeddings=normalize, convert_to_numpy=True
        )

        return embedding.tolist()

    def encode_queries(
        self, queries: List[str], normalize: bool = True, show_progress: bool = True
    ) -> List[List[float]]:
        """
...
if __name__ == "__main__":
    ...
    print(
        f"Результат: {len(test_embeddings)} embedding размерностью {len(test_embeddings[0])}"
    )
