...
После в настройках среды выполнения Google Colab в качестве аппаратного ускорителя был выбран графический процессор, так как использование GPU требуется условиями задания и необходимо для эффективного обучения сверточной нейронной сети.
...
_Рисунок 1: Информация о доступном графическом процессоре_

Далее было определено вычислительное устройство, на котором будет выполняться обучение нейронной сети. Результат выполнения команды показал, что в качестве устройства выбран cuda, что подтверждает успешное использование графического процессора для дальнейших вычислений.
...
В конце выводится пример изображения из обучающей подвыборки как и в прошлой лабораторной работе (рис. 2).

![Рисунок 2](img/2.png)

_Рисунок 2: Пример изображения из обучающей выборки_
...
_Рисунок 3: Вывод сводной информации о параметрах и размере выходов каждого слоя_
...
_Рисунок 4: Графики изменения функции потерь и точности классификации_
...
Из полученного отчёта (рис. 5) видно, что на обучающей выборке модель показывает идеальные результаты. Это означает, что модель полностью выучила обучающие данные и произошло переобучение.

![Рисунок 5](img/5.png)

_Рисунок 5: Отчёт о качестве классификации по классам на обучающей и тестовой выборках_
...
Реализуем для нашей модели данный пуллинг увеличивая шаг второго сверточного слоя до 4 и получаем такие результаты (рис. 6):

![Рисунок 6](pictures/08.png)

_Рисунок 6: Пуллинг с помощью шага свёртки (stride)_

Модель с пуллингом через увеличение шага свёртки показала почти идеальное обучение на тренировочной выборке с точностью около 0.886, что говорит о том, что она хорошо усвоила признаки обучающих данных. Однако на тестовой выборке точность 1. Это указывает на то, что модель всё ещё переобучается.
...
С помощью AdaptiveMaxPool2d уменьшаем размер каждой карты признаков до фиксированного размера, выбирая при этом наиболее выраженный признак в каждом канале и получаем такие результаты (рис. 7)

![Рисунок 7](img/07.png)

_Рисунок 7: Макс пуллинг_

На тренировочной выборке модель достигла идеальной точности, что говорит о хорошем обучении на исходных данных, но также о возможном переобучении. На тестовой выборке точность составила 0.916, что лучше, чем у Stride pooling. Макс-пулинг помогает лучше выделять ключевые признаки и улучшает обобщающую способность модели по сравнению с использованием stride в качестве пуллинга.
...
С помощью AdaptiveAvgPool2d уменьшаем размер каждой карты признаков до фиксированного размера, усредняя значения по каждому каналу и получаем такие результаты (рис. 8)

![Рисунок 8](img/5.png)

_Рисунок 8: Усредняющий пуллинг_

Модель достигла высокой точности на train 1 и нет признаков сильного перебучения. На test выборке результат 0.923, что показывает хорошее обобщение на новые данные. Усредняющий пуллинг сглаживает информацию по карте признаков, что снижает риск переобучения и делает классификацию более стабильной. В целом, Average pooling обеспечивает баланс между выделением признаков и сохранением обобщающей способности, делая модель устойчивой к шумам и мелким вариациям изображений.

В ходе экспериментов с тремя методами пуллинга мы увидели, что Stride pooling позволяет быстро уменьшать размер карт признаков за счёт шага свёртки, достигая почти идеальных результатов на обучающей выборке, но показывает более низкую точность на тесте 0.886. Макс-пулинг выделяет наиболее выраженные признаки и демонстрирует лучшее обобщение на тестовой выборке 0.916, но часть классов всё ещё распознаётся менее точно. Усредняющий пуллинг (Average pooling) показал наилучший результат: точность на тестовой выборке составила 0.923, при этом модель сохраняет устойчивость к шумам и мелким вариациям изображений. По полученным результатам усредняющий пуллинг обеспечивает более стабильное распознавание всех классов и наилучшую обобщающую способность модели на новых данных.
